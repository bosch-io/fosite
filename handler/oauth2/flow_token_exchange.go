...
// TokenExchangeGrantHandler is a response handler for the Token Exchange grant using the explicit grant type
...
func (c *TokenExchangeGrantHandler) HandleTokenEndpointRequest(ctx context.Context, request fosite.AccessRequester) error {
	...
	if or.GetDelegatingClient() == nil {
		...
		delegatingClient, _ = c.Store.GetClient(ctx, or.GetClient().GetID())
	} else {
		delegatingClient, _ = c.Store.GetClient(ctx, or.GetDelegatingClient().GetID())
	}
