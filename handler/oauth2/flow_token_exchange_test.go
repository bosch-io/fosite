	...
	for k, c := range []struct {
		...
		t.Run(fmt.Sprintf("case=%d", k), func(t *testing.T) {
			...
			if c.expectErr != nil {
				...
				require.Equal(t, errors.Cause(c.expectErr).(*fosite.RFC6749Error).Hint, errors.Cause(err).(*fosite.RFC6749Error).Hint)
			} else {
	...
	for k, c := range []struct {
		...
		},
		{
			...
			mock: func() {
				...
				chgen.EXPECT().GenerateAccessToken(nil, areq).Return("tokenfoo.bar", "bar", nil)
				refresh.EXPECT().GenerateRefreshToken(nil, areq).Return("refreshfoo.bar", "", nil)
				store.EXPECT().CreateAccessTokenSession(nil, "bar", gomock.Eq(areq.Sanitize([]string{}))).Return(nil)
